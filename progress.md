...
## 修复记录
- 2026-09-01 **结案：v1.0.5 微信实测可玩（score 13 截图铁证，T:7/7 P:7/7）**。此前"反复修不好"真相：v1.0.3 修复已生效，但用户 v1.0.3 后均在 **Safari** 测试，而该 iPhone 的 Safari 存在**触摸零派发**环境个案（diag.html 纯测试页 T:0 P:0 C:0，无任何游戏代码也收不到事件；微信 WKWebView 一切正常）。game03 另做三通道输入（touch/pointer/click + 400ms 去重，v1.0.5）。Safari 排查方向（用户侧）：彻底重启 Safari / 关"请求桌面网站" / 辅助功能-触摸调节。hub 微信引导文案已改为"微信内直接可玩"。诊断工具链：事件探针诊断条（inject_pwa.py v5）+ diag.html 触摸测试页（保留在站内）。
